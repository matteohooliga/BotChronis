...
class SetupView(discord.ui.View):
    ...
    def update_components(self):
        ...
        b3=discord.ui.Button(label=t['setup_btn_save'], custom_id="save", style=discord.ButtonStyle.success); b3.callback=self.cb_save; self.add_item(b3)
    ...
    async def refresh(self, i):
        t=config.TRANSLATIONS[self.sel_lang]
        e=discord.Embed(title=t['setup_panel_title'], description=self.get_desc(t), color=config.BOT_COLOR)
        e.set_footer(text=config.EMBED_FOOTER)
    ...
    def get_desc(self, t):
        ...
        return f"Lang: {self.sel_lang}\nSvc: {svc}\nLog: {log}\nDir: {rol}\nAuto: {aut}\nGoal: {int(self.sel_goal/3600000)}h"
    ...
    async def interaction_check(self, i):
        ...
        elif cid=="log": self.sel_logs=vals[0]
        elif cid=="role": self.sel_role=[int(v) for v in vals]
        elif cid=="autorole": self.sel_autorole=[int(v) for v in vals]
        if cid in ["svc", "log", "role", "autorole"]: await i.response.defer(); await self.refresh(i); return False
        return True