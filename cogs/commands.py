...
class ServiceCommands(commands.Cog):
    ...
    async def setup(self, interaction: discord.Interaction):
        ...
        async def scb(i, v):
            t = config.TRANSLATIONS[v.sel_lang]
            if not v.sel_service: return await i.response.send_message(t['setup_err_no_service'], ephemeral=True)
            await i.response.send_modal(GoalModal(v, t))
        view.children[2].callback = lambda i: scb(i, view)
    ...
    async def presence(self, interaction: discord.Interaction, channel: discord.TextChannel = None):
        if channel:
            try:
                ...
                embed = discord.Embed(title="Présence Réaction", color=config.BOT_COLOR)
                for reaction in message.reactions:
                    users = [u.mention async for u in reaction.users()]
                    if users: embed.add_field(name=str(reaction.emoji), value=", ".join(users))
                await interaction.response.send_message(embed=embed, ephemeral=False)
            except: return await interaction.response.send_message("Erreur message", ephemeral=True)
        else:
